 ...
 * @author TheANARXIST
 */
public class DangerBomb extends GameObjectParent implements IGameObject {

    private DangerBombState dangerBombState;

    public DangerBomb(int x, int y, int state) {
        super(x, y);
        this.fruitImageInit = new ImageIcon("images/fn_bomb_1.png");
        this.currentImage = this.fruitImageInit;
        if (state == 0) {
            dangerBombState = new DangerBombClassic();
        } else if (state == 1) {
            dangerBombState = new DangerBombArcade();
        }
    }
    ...
    public void slice() {
        this.sliced = true;
        dangerBombState.slice();
    }

    @Override
    public GameObjects getObjectType() {
        return GameObjects.DANGERBOMB;
    }
